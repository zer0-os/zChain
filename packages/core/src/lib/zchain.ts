...
export class ZCHAIN {
    ...
    async initialize (name: string, listenAddrs?: string[]): Promise<ILibp2p> {
      ...
      const cApi = getApi({ rest: beaconRestApiServerOpts }, {
        node: this.node
      });
  
  
      const restApi = new BeaconRestApiServer(beaconRestApiServerOpts, {
        api: cApi,
      } as any);
      console.log("api ", cApi);
      //console.log("RESTT ", restApi);
