...
export class ZCHAIN extends EventEmitter {
  ...
  async _getIPFSOptions(listenAddrs?: string[]) {
    ...
    const starAddresses = options.addresses.listen.filter((a) =>
      a.includes("p2p-webrtc-star")
    );
    if (starAddresses.length) {
      addWebRTCStarAddrs(options);
    }
  ...
  async startDaemon(
    ...
    if (fs.existsSync(path.join(os.homedir(), "/.jsipfs", "config"))) {
      const config = fs.readFileSync(
        path.join(os.homedir(), "/.jsipfs", "config"),
        "utf-8"
      );
      ...
      if (this.zId.peerId.toB58String() !== peerIdStr) {
        throw new Error(
          chalk.red(
            `Config mismatch :: Trying to start a new daemon, but a config is already present at ~/.jsipfs for a different zId(${peerIdStr}). Pease use --force to override`
          )
        );
      }
    }
  ...
  async load(): Promise<void> {
    if (!isDaemonOn()) {
      throw new Error(
        chalk.red(
          `Daemon not initialized at ~/.jsipfs. Please run "meow daemon .."`
        )
      );
    }
  ...
  async publish(
    ...
    await this.ipfs.pubsub.publish(channel, fromString(msg)).catch((err) => {
      throw new Error(err);
    });
