...
export class ZScreen {
    ...
    initStaticScreen() {
        this.mapBox = this.grid.set(6, 0, 6, 6, contrib.map, {
            label: "NODES MAP"
        });
        this.connectionsLogBox = this.grid.set(9, 6, 3, 6, contrib.log, {
            fg: "#2e5a94",
            selectedFg: "green",
            label: "DISCOVERY AND CONNECTIONS"
        });
        this.subscribedTopicsLog = this.grid.set(6, 6, 3, 6, blessed.log, {
            fg: "#cca843",
            selectedFg: "green",
            label: "CONSOLE LOGS"
        });
    ...
    drawTopicsBox(topics) {
        this.topicsWrapper = this.grid.set(0, 2, 6, 6, blessed.box, {
            label: "TOPICS"
        })
        this.topicsBox = blessed.list({
    ...
    drawOwnedZnasBox(ownedDomains) {
        ...
        if (formattedZnas.length > 0) {
            this.ownedZnasTable = this.grid.set(0, 2, 6, 6, contrib.table, {
                ...
                mouse: true,
                data: {
                    headers: ['Subdomain', 'Index', 'Token Id'],
                    data: formattedZnas
                },
                width: "shrink",
                style: {
    ...
    drawConnectionsBox(verifiedNodes) {
        this.verifiedNodesList = this.grid.set(0, 2, 6, 6, blessed.list, {
            ...
            selectedBg: "green",
            fg: "white",
            items: verifiedNodes.filter(function(item, pos, self) {
